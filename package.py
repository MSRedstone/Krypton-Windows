...
# found in the LICENSE file.
"""
Krypton packaging script for Microsoft Windows
"""
...
def _get_release_revision():
    revision_path = Path(__file__).resolve().parent / 'Krypton' / 'revision.txt'
...
def main():
    ...
    shutil.copyfile('build/src/out/Default/mini_installer.exe',
        'build/Krypton_{}-{}.{}_installer_{}.exe'.format(
    ...
    output = Path('build/Krypton_{}-{}.{}_windows_{}.zip'.format(
        get_chromium_version(), _get_release_revision(),
        _get_packaging_revision(), _get_target_cpu(build_outputs)))
