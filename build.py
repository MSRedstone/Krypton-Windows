...
# Copyright (c) 2026 The Krypton Authors. All rights reserved.
...
# found in the LICENSE file.
"""
Krypton build script for Microsoft Windows
"""
...
sys.path.insert(0, str(Path(__file__).resolve().parent / 'Krypton' / 'utils'))
import downloads
...
def main():
    ...
    if not args.ci or not (source_tree / 'BUILD.gn').exists():
        ...
        if args.tarball:
            ...
            download_info = downloads.DownloadInfo([_ROOT_DIR / 'Krypton' / 'downloads.ini'])
            ...
            # Clone sources
            subprocess.run([sys.executable, str(Path('Krypton', 'utils', 'clone.py')), '-o', 'build\\src', '-p', 'win32' if args.x86 else 'win-arm64' if args.arm else 'win64'], check=True)
        ...
        # Prune binaries
        pruning_list = (_ROOT_DIR / 'Krypton' / 'pruning.list') if args.tarball else (_ROOT_DIR  / 'pruning.list')
        ...
        patches.apply_patches(
            patches.generate_patches_from_series(_ROOT_DIR / 'Krypton' / 'patches', resolve=True),
            source_tree,
        ...
        # Substitute domains
        domain_substitution_list = (_ROOT_DIR / 'Krypton' / 'domain_substitution.list') if args.tarball else (_ROOT_DIR  / 'domain_substitution.list')
        domain_substitution.apply_substitution(
            _ROOT_DIR / 'Krypton' / 'domain_regex.list',
    ...
    if not args.ci or not (source_tree / 'out/Default').exists():
        ...
        gn_flags = (_ROOT_DIR / 'Krypton' / 'flags.gn').read_text(encoding=ENCODING)
        gn_flags += '\n'
